/**
 * NEXUS TRADE — TradingView-Grade Chart Engine
 * src/js/chart/chart.js
 *
 * FIXES NEEDED (known bugs):
 * - [ ] Crosshair price/time labels accumulate without clearing
 * - [ ] RSI line draws per-segment causing color gaps
 * - [ ] Fib drawing doesn't persist after scroll/zoom
 * - [ ] HA candles not recalculated on pair switch
 * - [ ] Zoom doesn't center on mouse position
 * - [ ] Live tick can drift far from real price over time
 */
...
const CFG = {
  ...
  PW:    62,   // price axis width (px)
  TH:    20,   // time axis height (px)
  SUB:   0,    // SUB=0: main chart fills full height; sub is separate canvas
  VOL:   0.12, // volume bars as fraction of chart height
};
...
const ST = {
  candles:     [],
  ha:          [],
  type:        'candle', // candle|hollow|bar|line|area|heikin
  sub:         'rsi',    // rsi|macd|stoch
  tf:          15,       // minutes per candle
  inds:        { ma20: false, ma50: false, bb: false, vwap: false },
  tool:        'cursor', // cursor|hline|trend|fib|rect
  drawings:    [],
  drawStart:   null,
  rightOffset: 0,        // bars scrolled from right (0 = last bar at right edge)
  barsVis:     80,       // visible bar count
  mx: -1, my: -1,        // mouse position
  drag: false, dragX0: 0, dragOff0: 0,
  W: 0, H: 0, mH: 0,
  pMin: 0, pMax: 1,
...
// ─── DATA GENERATION ─────────────────────────────────────
/**
 * Generate realistic OHLCV candle data that drifts toward the real price.
 * BUG FIX NEEDED: After long sessions, ST.candles grows unbounded → memory leak
 * TODO: cap at 2000 candles and prune oldest
 */
function CH_gen(pair) {
  ...
  for (let i = 400; i >= 0; i--) {
    ...
    ST.candles.push({
      o: +o.toFixed(dp), h: +h.toFixed(dp),
      l: +l.toFixed(dp), c: +c.toFixed(dp),
      v: Math.floor(80 + Math.random() * 800),
      t: now - i * ST.tf * 60000,
    });
    p = c;
  }

  // Force last candle to match live price exactly
...
function CH_buildHA() {
  ...
  ST.candles.forEach((d, i) => {
    ...
    ST.ha.push({
      o: hO, h: Math.max(d.h, hO, hC),
      l: Math.min(d.l, hO, hC), c: hC, v: d.v, t: d.t,
    });
  });
}
...
// ─── INDICATORS ──────────────────────────────────────────
function CH_sma(arr, p) {
  return arr.map((_, i) =>
    i < p - 1 ? null : arr.slice(i - p + 1, i + 1).reduce((s, v) => s + v, 0) / p
  );
}

function CH_ema(arr, p) {
  const k = 2 / (p + 1);
  let e = arr[0];
  return arr.map((v, i) => { e = i === 0 ? v : v * k + e * (1 - k); return e; });
}

function CH_bb(cls, p = 20) {
  const ma = CH_sma(cls, p);
  return cls.map((_, i) => {
    if (i < p - 1) return null;
    const sl = cls.slice(i - p + 1, i + 1), m = ma[i];
    const sd = Math.sqrt(sl.reduce((a, v) => a + (v - m) ** 2, 0) / p);
    return { u: m + 2 * sd, m, l: m - 2 * sd };
  });
}

function CH_rsi(cls, p = 14) {
...
function CH_macd(cls) {
  ...
  cls.forEach((v, i) => {
    ...
    const mc = e12 - e26;
    sg = i === 0 ? mc : mc * k9 + sg * (1 - k9);
    m.push(mc); s.push(sg); h.push(mc - sg);
  });
  return { m, s, h };
}

function CH_stoch(bars, p = 14) {
  return bars.map((_, i) => {
    if (i < p - 1) return null;
...
function CH_vwap(bars) {
  let cv = 0, cw = 0;
  return bars.map(b => { const tp = (b.h + b.l + b.c) / 3; cv += tp * b.v; cw += b.v; return cv / cw; });
}

// ─── LAYOUT ──────────────────────────────────────────────
function CH_niceStep(range, count) {
  const raw = range / count;
  const mag = Math.pow(10, Math.floor(Math.log10(raw)));
  for (const s of [1, 2, 2.5, 5, 10]) if (s * mag >= raw) return s * mag;
  return mag * 10;
}
...
function CH_layout() {
  ...
  ST.barsData   = all.slice(start, end);
  ST.startIdx   = start;
...
const pToY   = p  => ST.mH * (1 - (p - ST.pMin) / (ST.pMax - ST.pMin));
...
function CH_resize() {
  ...
  CH_ctx = mc.getContext('2d');
  CH_ctx.scale(CH_DPR, CH_DPR);
  ST.W = W; ST.H = H;
  if (subPanel && sc) {
    ...
    CH_subCtx = sc.getContext('2d');
    CH_subCtx.scale(CH_DPR, CH_DPR);
  }
}
...
function CH_drawMain() {
  ...
  while (pg <= ST.pMax) {
    ...
    if (y > 0 && y < mH) {
      ...
      ctx.fillText(pg.toFixed(dp), cW + 4, y + 3);
    }
    pg += pStep;
  }
  ...
  bars.forEach((b, i) => {
    ...
    ctx.fillStyle = b.c >= b.o ? 'rgba(38,166,154,0.2)' : 'rgba(239,83,80,0.2)';
    ctx.fillRect(x - ST.barW / 2, mH - vH, ST.barW, vH);
  });

  // Bollinger Bands
  if (ST.inds.bb) {
    const all = CH_data(), cls = all.map(b => b.c);
    const bb = CH_bb(cls, 20).slice(ST.startIdx, ST.startIdx + bars.length);
    // Fill
    ctx.beginPath();
    bb.forEach((v, i) => { if (v) ctx.lineTo(xOfBar(i), pToY(v.u)); });
  ...
  const all = CH_data(), cls = all.map(b => b.c);
  [
    [ST.inds.ma20, 20, '#f5c518', 1.5],
    [ST.inds.ma50, 50, '#8b5cf6', 1.5],
  ].forEach(([on, p, col, w]) => {
    if (!on) return;
  ...
  if (ST.type === 'line' || ST.type === 'area') {
    ...
    if (ST.type === 'area') {
      ctx.lineTo(xOfBar(bars.length - 1), mH); ctx.lineTo(xOfBar(0), mH); ctx.closePath();
    ...
    bars.forEach((b, i) => {
      const x = xOfBar(i), up = b.c >= b.o;
      const uc = CFG.UP, dc = CFG.DN;
  ...
  if (last) {
    ...
    ctx.fillStyle = 'rgba(0,212,255,0.9)'; ctx.fillRect(cW, ly - 9, CFG.PW, 18);
    ctx.fillStyle = '#020810'; ctx.font = 'bold 9px JetBrains Mono, monospace'; ctx.textAlign = 'center';
    ctx.fillText(last.c.toFixed(dp), cW + CFG.PW / 2, ly + 3);
    const chPrice = document.getElementById('chPrice');
    const tradePriceLive = document.getElementById('tradePriceLive');
    if (chPrice) chPrice.textContent = last.c.toFixed(dp);
    if (tradePriceLive) tradePriceLive.textContent = last.c.toFixed(dp);
  }

  // Crosshair
  if (ST.mx >= 0 && ST.mx <= cW && ST.my >= 0 && ST.my <= mH) {
    ctx.strokeStyle = CFG.CROSS; ctx.lineWidth = 0.8; ctx.setLineDash([4, 4]);
    ctx.beginPath(); ctx.moveTo(ST.mx, 0); ctx.lineTo(ST.mx, mH); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(0, ST.my); ctx.lineTo(cW, ST.my); ctx.stroke();
    ctx.setLineDash([]);

    // Price label on Y axis
    const hp = ST.pMax - (ST.pMax - ST.pMin) * (ST.my / mH);
    ctx.fillStyle = 'rgba(0,55,75,0.95)'; ctx.fillRect(cW, ST.my - 9, CFG.PW, 18);
    ctx.fillStyle = '#e0f4ff'; ctx.font = '9px JetBrains Mono, monospace'; ctx.textAlign = 'center';
    ...
    if (bars[bi]) {
      ...
      // OHLC update
      const b = bars[bi];
      const ids = { o: 'chO', h: 'chH', l: 'chL', v: 'chV' };
      if (document.getElementById(ids.o)) {
        document.getElementById(ids.o).textContent = b.o.toFixed(dp);
        document.getElementById(ids.h).textContent = b.h.toFixed(dp);
        document.getElementById(ids.l).textContent = b.l.toFixed(dp);
        document.getElementById(ids.v).textContent = (b.v / 1000).toFixed(1) + 'K';
      }
    }
  }
...
function CH_drawSub() {
  ...
  if (ST.sub === 'rsi') {
    ...
    // Level lines
    [[30, 'rgba(239,83,80,0.15)'], [50, 'rgba(100,140,170,0.1)'], [70, 'rgba(239,83,80,0.15)']].forEach(([lv, c]) => {
      const y = H * (1 - lv / 100);
      ctx.strokeStyle = c; ctx.lineWidth = 0.8; ctx.setLineDash([3, 4]);
      ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(cW, y); ctx.stroke(); ctx.setLineDash([]);
      ctx.fillStyle = 'rgba(80,120,150,0.6)'; ctx.font = '8px JetBrains Mono, monospace'; ctx.textAlign = 'left';
      ctx.fillText(lv, cW + 3, y + 3);
    });
    // RSI line (single continuous path for performance)
    // TODO FIX: color-coding per-value currently breaks line continuity
    let lastR = 50;
    ctx.beginPath(); ctx.strokeStyle = 'rgba(0,212,255,0.8)'; ctx.lineWidth = 1.2;
    rsi.forEach((v, i) => {
    ...
    ctx.fillStyle = lastR > 70 ? CFG.DN : lastR < 30 ? CFG.UP : 'rgba(0,212,255,0.8)';
    ctx.fillRect(cW + 2, ry - 7, CFG.PW - 4, 14);
    ctx.fillStyle = '#030810'; ctx.font = 'bold 9px JetBrains Mono, monospace'; ctx.textAlign = 'center';
    ...
    hV.forEach((v, i) => {
      const x = xB(i), y = yV(v), w = Math.max(1, bSp * 0.7);
      ctx.fillStyle = v >= 0 ? 'rgba(38,166,154,0.5)' : 'rgba(239,83,80,0.5)';
      ctx.fillRect(x - w / 2, Math.min(y, mid), w, Math.abs(y - mid));
    });
    ...
    st.forEach((v, i) => {
      if (v === null) return;
      const x = xB(i), y = H * (1 - v / 100);
      i === 0 || st[i - 1] === null ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
    }); ctx.stroke();
...
function CH_renderDrawings(ctx, cW, dp) {
  ST.drawings.forEach(d => {
    ...
    if (d.type === 'hline') {
      ...
      ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(cW, y); ctx.stroke();
      ctx.fillStyle = d.color || 'rgba(255,200,0,0.8)'; ctx.font = '9px JetBrains Mono, monospace'; ctx.textAlign = 'right';
      ...
      [0, 0.236, 0.382, 0.5, 0.618, 0.786, 1].forEach((lv, fi) => {
        ...
        ctx.fillStyle = cols[fi]; ctx.font = '8px JetBrains Mono, monospace'; ctx.textAlign = 'right';
...
function CH_liveTick() {
  ...
  const dp   = cp.d || 5;
  const last = ST.candles[ST.candles.length - 1];
  last.c = +(Math.max(0.001, last.c + (Math.random() - 0.49) * last.c * 0.0004)).toFixed(dp);
  last.h = Math.max(last.h, last.c);
  last.l = Math.min(last.l, last.c);
  if (Math.random() < 0.06) {
    const o = last.c;
    const c = +(o + (Math.random() - 0.49) * o * 0.0005).toFixed(dp);
    ST.candles.push({ o, h: Math.max(o, c) + Math.random() * Math.abs(c - o) * 0.3, l: Math.min(o, c) - Math.random() * Math.abs(c - o) * 0.3, c, v: Math.floor(50 + Math.random() * 500), t: Date.now() });
...
function CH_bindEvents() {
  ...
  area.addEventListener('mousemove', e => {
    ...
    if (ST.drag) {
      const dx = e.clientX - ST.dragX0;
      const delta = Math.round(dx / (ST.barSpacing || 8));
      ST.rightOffset = Math.max(0, Math.min(ST.candles.length - 5, ST.dragOff0 - delta));
    }
    CH_dirty = true;
  });
  ...
  area.addEventListener('mousedown', e => {
    if (ST.tool === 'cursor') {
      ST.drag = true; ST.dragX0 = e.clientX; ST.dragOff0 = ST.rightOffset;
      area.style.cursor = 'grabbing';
    } else {
      const r = area.getBoundingClientRect();
      const mx = e.clientX - r.left, my = e.clientY - r.top;
      const bi = ST.startIdx + Math.max(0, Math.min(ST.barsData.length - 1, barAtX(mx)));
      const p  = ST.pMax - (ST.pMax - ST.pMin) * (my / ST.mH);
      if (ST.tool === 'hline') {
        ST.drawings.push({ type: 'hline', price: p, color: 'rgba(255,200,0,0.85)' });
        CH_dirty = true;
      } else {
        ST.drawStart = { b: bi, p };
      }
  ...
  area.addEventListener('wheel', e => {
    e.preventDefault();
    ST.barsVis = Math.max(5, Math.min(300, Math.round(ST.barsVis * (e.deltaY < 0 ? 0.82 : 1.22))));
    CH_dirty = true;
  }, { passive: false });
...
function CH_setSub(type, el) {
  ST.sub = type;
  document.querySelectorAll('#subRSI,#subMACD,#subSTOCH').forEach(b => b.classList.remove('active', 'act'));
...
function CH_setTool(t, el) {
  ...
  if (el) el.classList.add('act');
  const area = document.getElementById('mainChartArea');
  if (area) area.style.cursor = t === 'cursor' ? 'default' : 'crosshair';
}
...
function CH_screenshot() {
  const c = document.getElementById('tvCanvas'); if (!c) return;
  const a = document.createElement('a');
  a.href = c.toDataURL('image/png');
  a.download = `NEXUS-${S.pair.replace('/', '_')}-${Date.now()}.png`;
  a.click(); showToast('📷 Screenshot saved!');
}

// Indicator panel
const INDS = [
  { k: 'ma20', l: 'MA (20)',           c: '#f5c518' },
  { k: 'ma50', l: 'MA (50)',           c: '#8b5cf6' },
  { k: 'bb',   l: 'Bollinger Bands',  c: 'rgba(0,200,255,0.6)' },
  { k: 'vwap', l: 'VWAP',             c: '#ff9500' },
];
function CH_initIndPanel() {
  ...
  el.innerHTML = INDS.map(ind => `
    <div class="ind-row" onclick="CH_toggleInd('${ind.k}',this)">
      ...
      <div style="width:16px;height:2px;background:${ind.c};border-radius:1px"></div>
    </div>`).join('');
}
...
function setTF(tf, el) {
  const map = { '1m': 1, '5m': 5, '15m': 15, '1H': 60, '4H': 240, 'يومي': 1440 };
  CH_setTF(map[tf] || 15, el);
}
